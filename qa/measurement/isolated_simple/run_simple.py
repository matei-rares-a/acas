"""
Runner izolat — OAuth2 Simple pe port 5100, fara competitie cu alte servere.

Utilizare:
    python qa/measurement/isolated/run_simple.py [users] [runtime]

Exemple:
    python qa/measurement/isolated/run_simple.py 100 60s
    python qa/measurement/isolated/run_simple.py 400 120s
"""
import subprocess
import sys
import time
...
def main():
    ...
    csv_prefix = str(_GENERATED / "locust_simple")
    html_out   = str(_GENERATED / "locust_simple_report.html")
    ...
    try:
        ...
        subprocess.run(
            [
                "locust",
                "-f",            str(_ISOLATED_DIR / "locust_simple.py"),
                f"--users={users}",
                "--spawn-rate=20",
                "--headless",
                f"--run-time={runtime}",
