"""
Runner izolat — OAuth2 PKCE pe port 5400, fara competitie cu alte servere.

Utilizare:
    python qa/measurement/isolated_pkce/run_pkce.py [users] [runtime]

Exemple:
    python qa/measurement/isolated_pkce/run_pkce.py 100 60s
    python qa/measurement/isolated_pkce/run_pkce.py 400 120s
"""
import subprocess
import sys
import time
...
def main():
    ...
    csv_prefix = str(_GENERATED / "locust_pkce")
    html_out   = str(_GENERATED / "locust_pkce_report.html")
    ...
    try:
        ...
        subprocess.run(
            [
                "locust",
                "-f",            str(_ISOLATED_DIR / "locust_pkce.py"),
                f"--users={users}",
                "--spawn-rate=20",
                "--headless",
                f"--run-time={runtime}",
